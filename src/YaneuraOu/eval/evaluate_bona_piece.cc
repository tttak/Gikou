...
namespace Eval
{
  ...
  std::string pretty(YanePiece pc) {
  ...
  std::ostream& operator<<(std::ostream& os, BonaPiece bp)
  {
    ...
    {
      for (auto c : COLOR)
        for (YanePiece pc = PAWN; pc < KING; ++pc)
        {
          // この駒種の上限(e.g. 歩 = 18)
      ...
      for (auto pc : YanePiece())
        if (kpp_board_index[pc].fb <= bp && bp < kpp_board_index[pc].fb + SQ_NB)
        {
#ifdef PRETTY_JP
          os << Square(bp - kpp_board_index[pc].fb).ToSfen() << pretty(pc);
#else
          os << Square(bp - kpp_board_index[pc].fb).ToSfen() << pc;
#endif
          break;
        }
    }
